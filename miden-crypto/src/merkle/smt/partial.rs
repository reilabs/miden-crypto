...
use crate::{
    EMPTY_WORD, Word,
    merkle::{
        EmptySubtreeRoots, InnerNodeInfo, MerkleError, NodeIndex, SparseMerklePath,
...
impl PartialSmt {
    ...
    fn is_leaf_tracked(&self, key: &Word) -> bool {
        ...
        // Check if the leaf can be proven empty by traversing up to the root.
        // Optimization: when traversing through empty subtrees (no stored inner node),
        // we use precomputed empty subtree roots instead of computing hashes.
        let mut index: NodeIndex = leaf_index.into();
        let mut current_hash = *EmptySubtreeRoots::entry(SMT_DEPTH, SMT_DEPTH);

        while index.depth() > 0 {
            let parent_index = index.parent();

            match self.0.inner_nodes.get(&parent_index) {
                Some(inner_node) => {
                    // Explicit inner node found - verify our hash matches the stored child.
                    // If it matches, we're connected to a verified path (the proof was verified
                    // against the root when added), so we can return early.
                    let stored_child_hash = if index.is_value_odd() {
                        inner_node.right
                    } else {
                        inner_node.left
                    };

                    return current_hash == stored_child_hash;
                },
                None => {
                    // No inner node stored - this subtree must be empty.
                    // Verify our hash matches the empty subtree root at this depth.
                    let empty_at_depth = *EmptySubtreeRoots::entry(SMT_DEPTH, index.depth());
                    if current_hash != empty_at_depth {
                        return false;
                    }

                    // Move up using precomputed empty subtree root (no hash computation!)
                    current_hash = *EmptySubtreeRoots::entry(SMT_DEPTH, parent_index.depth());
                },
            }
