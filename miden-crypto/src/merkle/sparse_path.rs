...
impl Iterator for SparseMerkleIter {
    ...
    fn next(&mut self) -> Option<RpoDigest> {
        ...
        match self.path.get(self.tree_depth, next_depth) {
            Some(node) => {
...
mod tests {
    ...
    fn iterator() {
        ...
        for (i, (key, _value)) in tree.entries().enumerate() {
            ...
            let sparse_path = SparseMerklePath::from_path(SMT_DEPTH, path.clone()).unwrap();
            ...
            for (depth, iter_node) in sparse_path.clone().into_iter().enumerate() {
                let control_node = sparse_path.get(SMT_DEPTH, depth as u8).unwrap();
