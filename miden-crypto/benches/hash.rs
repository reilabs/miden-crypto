...
use miden_crypto::{
    Felt, Word,
    hash::{blake::Blake3_256, keccak::Keccak256, poseidon2::Poseidon2, rpo::Rpo256, rpx::Rpx256},
};
...
fn keccak256_2to1(c: &mut Criterion) {
    let v: [<Keccak256 as Hasher>::Digest; 2] =
        [Keccak256::hash(&[1_u8]), Keccak256::hash(&[2_u8])];
    c.bench_function("Keccak256 2-to-1 hashing (cached)", |bench| {
        bench.iter(|| Keccak256::merge(black_box(&v)))
    });

    c.bench_function("Keccak256 2-to-1 hashing (random)", |bench| {
        bench.iter_batched(
            || {
                [
                    Keccak256::hash(&rand_value::<u64>().to_le_bytes()),
                    Keccak256::hash(&rand_value::<u64>().to_le_bytes()),
                ]
            },
            |state| Keccak256::merge(&state),
            BatchSize::SmallInput,
        )
    });
}

fn keccak256_sequential(c: &mut Criterion) {
    let v: [Felt; 100] = (0..100)
        .map(Felt::new)
        .collect::<Vec<Felt>>()
        .try_into()
        .expect("should not fail");
    c.bench_function("Keccak256 sequential hashing (cached)", |bench| {
        bench.iter(|| Keccak256::hash_elements(black_box(&v)))
    });

    c.bench_function("Keccak256 sequential hashing (random)", |bench| {
        bench.iter_batched(
            || {
                let v: [Felt; 100] = (0..100)
                    .map(|_| Felt::new(rand_value()))
            ...
            |state| Keccak256::hash_elements(&state),
            BatchSize::SmallInput,
        )
    });
}
